                ...
                switch ($node->getAttribute('type')) {
                    ...
                    case 'media':
                        ...
                        switch ($node->getChild('enc')->getAttribute('mediatype')) {
                            case 'image':
                                file_put_contents('image.bin', $plaintext);
                            ...
                            case 'audio':
                                try {
                                    file_put_contents('audio.bin', $plaintext);
                                    $audio = new AudioMessage();
                                    $audio->parseFromString($plaintext);
                                    $keys = (new HKDFv3())->deriveSecrets($audio->getRefKey(), hex2bin('576861747341707020496d616765204b657973'), 112);
                                    $iv = substr($keys, 0, 16);
                                    $keys = substr($keys, 16);
                                    $parts = str_split($keys, 32);
                                    $key = $parts[0];
                                    ...
                                    file_put_contents('test.oga', $decrypted_audio);
                                    $child = new ProtocolNode('media',
                                        [
                                            'size'     => $audio->getSize(),
                                            'url'      => $audio->getUrl(),
                                            'mimetype' => current($mimeParts),
                                            'filehash' => bin2hex($audio->getSha256()),
                                            'seconds'  => $audio->getDuration(),
                                            'acodec'   => isset($mimeParts[1]) ? trim($mimeParts[1]) : '',
                            ...
                            case 'video':
                                try {
                                    file_put_contents('video.bin', $plaintext);
                                    $video = new VideoMessage();
                                    $video->parseFromString($plaintext);
                                    $keys = (new HKDFv3())->deriveSecrets($video->getRefKey(), hex2bin('576861747341707020496d616765204b657973'), 112);
                                    $iv = substr($keys, 0, 16);
                                    $keys = substr($keys, 16);
                                    $parts = str_split($keys, 32);
                                    $key = $parts[0];
                                    ...
                                    //should be changed to nice curl, no extra headers :D
                                    $response = self::$client->get($video->getUrl());
                                    $file_enc = $response->getBody()->getContents();

                                    //requires mac check , last 10 chars
                                    $mac = substr($file_enc, -10);
                                    $cipherVideo = substr($file_enc, 0, strlen($file_enc) - 10);
                                    $decrypted_video = pkcs5_unpad(mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $key, $cipherVideo, MCRYPT_MODE_CBC, $iv));

                                    $child = new ProtocolNode('media',
                                        [
                                            'url'       => $video->getUrl(),
                                            'file'      => $decrypted_video ?: $file_enc,
                                            'size'      => $video->getSize(),
                                            'mimetype'  => $video->getMimeType(),
                                            'filehash'  => bin2hex($video->getSha256()),
                                            ...
                                            'abitrate'  => '',
                                            'type'      => 'video',
                                        ], null, $video->getThumbnail());
                                    $node->addChild($child);
                                } catch (Exception $exception) {
                                    dump($exception->getMessage(), $exception->getLine(), $exception->getTraceAsString());
                                    exit;
                                }
                                break;
                            case 'location':
